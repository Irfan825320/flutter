...
class BurgerView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<ProductProvider>(builder: (context, productProvider, child) {
      return productProvider.burgerList != null ? Column(children: [

        SizedBox(height: ResponsiveHelper.isDesktop(context) ? Dimensions.PADDING_SIZE_LARGE : 0),

        TitleWidget(title: getTranslated('burger_item', context), onTap: () {
          Navigator.pushNamed(context, RouteHelper.BurgerRoute());
        }),

        ResponsiveHelper.isDesktop(context)
            ?  GridView.builder(
                  itemCount: productProvider.burgerList.length >= 10 ? 10 : productProvider.burgerList.length,
                  ...
                  crossAxisSpacing: 13,
                    ...
                    itemBuilder: (context,index){
                      return listViewBurgerItem(context: context,index: index,productProvider: productProvider);
                    },
                  )
          ...
          child:  ListView.builder(
            itemCount: productProvider.burgerList.length,
  ...
  Widget listViewBurgerItem({BuildContext context, int index, ProductProvider productProvider}){
    return Padding(
      padding: EdgeInsets.only(right: Dimensions.PADDING_SIZE_SMALL),
      child: InkWell(
        onTap: () {
          Navigator.of(context).pushNamed(RouteHelper.getProductDetailsRoute(product: productProvider.burgerList[index]),
              arguments: ProductDetailsScreen(product: productProvider.burgerList[index]));
        },
        child: Container(
          width: 150,
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              child: ClipRRect(
                ...
                child: FadeInImage.assetNetwork(
                  ...
                  image: '${Provider.of<SplashProvider>(context, listen: false).baseUrls.productImageUrl}'
                      '/${productProvider.burgerList[index].image[0]}',
            ...
            Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(
                productProvider.burgerList[index].name,
                style: poppinsMedium.copyWith(fontSize: Dimensions.FONT_SIZE_SMALL),
                maxLines: 1, overflow: TextOverflow.ellipsis,
              ),

              Text(
                '${productProvider.burgerList[index].capacity} ${productProvider.burgerList[index].unit}',
                  ...
                  children: [
                    Text(
                      PriceConverter.convertPrice(
                        context, productProvider.burgerList[index].price,
                        discount: productProvider.burgerList[index].discount,
                        discountType: productProvider.burgerList[index].discountType,
                      ),
                      style: poppinsBold.copyWith(fontSize: Dimensions.FONT_SIZE_SMALL),
                    ),

                    productProvider.burgerList[index].discount > 0 ? Text(
                      PriceConverter.convertPrice(
                        context, productProvider.burgerList[index].price,
                      ),
