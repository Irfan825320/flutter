...
class ProductProvider extends ChangeNotifier {
  ...
  //Modified Here
  List<Product> _momoList;
  List<Product> _rollList;
  List<Product> _noodlesList;
  List<Product> _burgerList;
  ...
  List<Product> get momoList=>_momoList;
  List<Product> get rollList=>_rollList;
  List<Product> get noodlesList=>_noodlesList;
  List<Product> get burgerList=>_burgerList;
  //Modification Ends Here
  ...
  //Roll List
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      _hasData = _rollList.length > 1;
      List<Product> _products = [];
      _products.addAll(_rollList);
      List<double> _prices = [];
      _products.forEach((product) => _prices.add(double.parse(product.price.toString())));
      _prices.sort();
  ...
  //Noodles List
    void getNoodlesList(BuildContext context, String languageCode) async {
    _noodlesList = [];
    _hasData = true;
    ApiResponse apiResponse = await productRepo.getBrandOrCategoryProductList("3", languageCode);
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      _noodlesList = [];
      apiResponse.response.data.forEach((product) => _noodlesList.add(Product.fromJson(product)));
      _hasData = _noodlesList.length > 1;
      List<Product> _products = [];
      _products.addAll(_noodlesList);
      List<double> _prices = [];
      _products.forEach((product) => _prices.add(double.parse(product.price.toString())));
      _prices.sort();
      if(noodlesList.length!=0)
      _maxValue = _prices[_prices.length - 1];
  ...
  //Burger List
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      _hasData = _burgerList.length > 1;
      List<Product> _products = [];
      _products.addAll(_burgerList);
      List<double> _prices = [];
      _products.forEach((product) => _prices.add(double.parse(product.price.toString())));
      _prices.sort();
